...
export async function GET(request: NextRequest) {
  ...
  try {
    const snapshot = await buildStaffingIngestion({
      eventId,
      companyId: profile.company_id,
    })
    return NextResponse.json({ data: snapshot })
  } catch (error: any) {
    console.error('Failed to build staffing insights', error)
    return NextResponse.json({ error: 'Unable to load staffing insights' }, { status: 500 })
  }
}
