...
// ... [Hero Cards Configuration - Same as before] ...
const heroCards = [
  {
    id: 'incidents',
...
// ... [CardVisual Component - Same as before] ...
function CardVisual({ id }: { id: string }) {
  if (id === 'incidents') {
    return (
      <div className="flex flex-col gap-3 py-2">
        {[
          { label: 'Medical Assist', loc: 'Section 102', time: 'Now', status: 'bg-red-400' },
          { label: 'Crowd Density', loc: 'Gate A', time: '5m', status: 'bg-amber-400' },
          { label: 'Noise Complaint', loc: 'East Wing', time: '12m', status: 'bg-emerald-400' },
        ].map((inc, i) => (
          <div key={i} className="flex items-center gap-3 rounded-lg bg-black/20 p-3 backdrop-blur-md border border-white/10 shadow-sm">
            <div className={`relative flex h-2.5 w-2.5`}>
               <span className={`absolute inline-flex h-full w-full animate-ping rounded-full ${inc.status} opacity-75`}></span>
               <span className={`relative inline-flex h-2.5 w-2.5 rounded-full ${inc.status}`}></span>
            </div>
            <div className="flex-1 space-y-1.5">
              <div className="h-2 w-24 rounded-full bg-white/40" />
              <div className="h-1.5 w-12 rounded-full bg-white/20" />
            </div>
            <div className="text-[10px] font-mono text-white/60 bg-white/10 px-1.5 py-0.5 rounded border border-white/5">{inc.time}</div>
          </div>
        ))}
      </div>
    )
        ...
        {['Security', 'Medical', 'Logistics'].map((role, i) => (
          <div key={i} className="space-y-2">
            <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-white/70">
              <span>{role}</span>
              <span>{[94, 100, 88][i]}%</span>
            </div>
            <div className="h-2.5 w-full rounded-full bg-black/30 overflow-hidden backdrop-blur-sm ring-1 ring-white/10">
              <div 
                 className="h-full rounded-full bg-gradient-to-r from-white/90 to-white/70 shadow-[0_0_12px_rgba(255,255,255,0.4)]" 
                 style={{ width: `${[94, 100, 88][i]}%` }} 
              />
            </div>
          </div>
        ))}
  ...
  if (id === 'ai') {
    return (
      <div className="py-2">
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-b from-white/15 to-white/5 p-5 border border-white/10 shadow-inner">
          <div className="absolute top-3 right-3">
             <SparklesIcon className="w-5 h-5 text-white/40" />
          </div>
          <div className="space-y-3">
            <div className="h-2.5 w-3/4 rounded-sm bg-white/50 animate-pulse" />
            <div className="h-2 w-full rounded-sm bg-white/30" />
            <div className="h-2 w-5/6 rounded-sm bg-white/30" />
            <div className="h-2 w-4/5 rounded-sm bg-white/30" />
          </div>
          <div className="mt-5 flex items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-md bg-pink-500/20 px-2 py-1 border border-pink-200/20">
               <span className="h-1.5 w-1.5 rounded-full bg-pink-300"></span>
               <span className="text-[10px] font-medium text-pink-100">Insight Generated</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#F1F4F9] text-slate-900 font-sans selection:bg-blue-200">
      ...
      <section id="hero" className="relative overflow-hidden bg-[#23408e]">
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col items-center gap-12 px-4 py-16 sm:px-6 sm:py-24 md:px-10 lg:flex-row lg:justify-between lg:gap-16">
          ...
          <FadeIn delay={0.2} className="relative w-full max-w-[500px] flex-1 lg:max-w-[540px]">
            <div className="relative mx-auto" style={{ height: 520 }}>
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-blue-500/20 blur-3xl rounded-full -z-10 pointer-events-none"></div>
              ...
              >
                {heroCards.map((card) => (
                  <Card key={card.id} className="overflow-hidden rounded-2xl shadow-2xl ring-1 ring-white/10">
                    <div
                      className={`flex h-full w-full flex-col justify-between bg-gradient-to-br ${card.gradient} p-7 text-white`}
                    >
                      <div>
                        <div className="inline-flex items-center rounded-md bg-white/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white backdrop-blur-md ring-1 ring-white/20">
                          {card.badge}
                        </div>
                        <h3 className="mt-4 text-2xl font-bold tracking-tight">{card.title}</h3>
                        <p className="mt-2 text-sm font-medium text-white/70 leading-relaxed">{card.description}</p>
                      </div>
                      ...
                      <div className="mt-auto pt-4 border-t border-white/10">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold uppercase tracking-wide text-white/60">{card.statLabel}</span>
                          <span className="font-mono text-xl font-bold tracking-tight">{card.statValue}</span>
                        </div>
                      </div>
                    </div>
                  </Card>
