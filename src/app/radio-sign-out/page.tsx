...
export default function RadioSignOutPage() {
  ...
  const handleDeleteRadioLog = async (radioId: number) => {
    console.log('Delete button clicked for radio ID:', radioId);
    
    if (deletingId === radioId) {
      console.log('Already deleting this item, ignoring click');
      return;
    }
    
    if (!confirm('Are you sure you want to delete this radio log entry? This action cannot be undone.')) {
    ...
    try {
      console.log('Proceeding with deletion for radio ID:', radioId);
      ...
      if (error) {
        ...
        throw new Error(`Failed to delete radio log entry: ${error.message}`);
      }

      console.log('Successfully deleted radio log entry');
      
      // Refresh the data
      await fetchSignOuts();
      console.log('Data refreshed after deletion');
    } catch (err) {
      console.error('Error in handleDeleteRadioLog:', err);
        ...
        <div className="mt-10 bg-white dark:bg-[#23408e] rounded-2xl border border-gray-200 dark:border-[#2d437a] shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-[#2d437a]">
              ...
              <tbody className="bg-white dark:bg-[#23408e] divide-y divide-gray-200 dark:divide-[#2d437a]">
                {signOuts.map((radio) => (
                  <tr key={radio.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        onClick={() => {
                          console.log('Delete button clicked for radio:', radio.id);
                          handleDeleteRadioLog(radio.id);
                        }}
                        ...
                        title={deletingId === radio.id ? "Deleting..." : "Delete radio log entry"}
                      >
                        {deletingId === radio.id ? (
