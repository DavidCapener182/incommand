...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase
      ...
      .eq('incident_id', parseInt(id as string));
  ...
  if (req.method === 'POST') {
    ...
    const { error } = await supabase
      .from('incident_attachments')
      .insert([{ incident_id: parseInt(id as string), file_url, file_type, uploaded_by }]);
    if (error) return res.status(400).json({ error: error.message });

    const { error: eventError } = await supabase.from('incident_events').insert([
      { incident_id: parseInt(id as string), event_type: 'attachment', event_data: { file_url, file_type }, created_by: uploaded_by },
    ]);
